...
import { BaseSchema, BaseModuleSchema, LobbyPlayerSchema, LobbyRoomSchema, ResourceNodeSchema, SpaceState, UnitSchema, } from "@space-combat/shared";
...
export class SpaceRoom extends Colyseus.Room {
    constructor() {
        super(...arguments);
        this.stats = UNIT_STATS;
        this.unitConfig = UNIT_CONFIG;
    ...
    onCreate() {
        ...
        this.onMessage("base:build", (client, payload) => {
            this.handleBuildRequest(client, payload);
        });
        this.onMessage("base:purchaseModule", (client, payload) => {
            this.handleModulePurchase(client, payload);
        });
        this.onMessage("module:visit", (client, payload) => {
            this.handleModuleVisit(client, payload);
        });
        this.onMessage("module:garageInstall", (client, payload) => {
            this.handleGarageInstallRequest(client, payload);
        });
        this.onMessage("module:techUpgrade", (client, payload) => {
            this.handleTechUpgrade(client, payload);
        });
        this.onMessage("lab:startResearch", (client, payload) => {
    ...
    getUnitStats(unit) {
        ...
        return {
            ...
            weaponDamage: weaponStats.weaponDamage + Math.max(0, unit.weaponDamageBonus ?? 0),
        };
    }
    tick(dtMs) {
        if (this.matchEnded) {
    ...
    finishDropoff(unit) {
        const base = this.state.bases.get(unit.orderTargetId) ??
    ...
    getClosestBaseForOwner(ownerId, unitX, unitZ) {
        let closest = null;
        for (const base of this.state.bases.values()) {
            if (base.owner !== ownerId) {
                continue;
            ...
            const closestDistance = Math.hypot(closest.x - unitX, closest.z - unitZ);
    ...
    handleModulePurchase(client, payload) {
    ...
    handleModuleVisit(client, payload) {
    ...
    handleGarageInstallRequest(client, payload) {
        ...
        const { cost, durationSeconds, applied } = this.getGarageInstallConfig(base, unit, installType, optionKey);
    ...
    getGarageInstallConfig(base, unit, installType, optionKey) {
    ...
    canInstallTechPackage(base, unit, techKey) {
    ...
    applyGarageInstall(unit) {
        ...
        const techKey = unit.pendingInstallKey;
        if ((unit.techSlotsUsed ?? 0) >= (unit.techMounts ?? 0)) {
            return;
        }
        switch (techKey) {
            case "SHIELDS":
                if (unit.techShieldPackage)
                    return;
    ...
    processGarageInstalls(dt) {
        for (const unit of this.state.units.values()) {
            ...
            if (unit.installState === "QUEUED") {
                unit.installState = "INSTALLING";
                unit.installTimeRemaining = Math.max(unit.installTimeRemaining, unit.pendingInstallDuration);
            }
            unit.orderType = "STOP";
            unit.orderTargetId = "";
            unit.vx = 0;
    ...
    handleTechUpgrade(client, payload) {
        const baseId = payload?.baseId;
        ...
        base.resourceStock -= cost;
        if (upgradeType === "STORAGE") {
            base.collectorStorageBonus = Math.min(COLLECTOR_STORAGE_MAX_BONUS, base.collectorStorageBonus + COLLECTOR_STORAGE_UPGRADE_STEP);
        }
    ...
    processResearch(dt) {
        for (const base of this.state.bases.values()) {
            ...
            base.activeResearchRemaining = Math.max(0, base.activeResearchRemaining - dt);
    ...
    areResearchPrerequisitesMet(base, prerequisites) {
    ...
    isWeaponResearched(base, weaponType) {
        switch (weaponType) {
            case "LASER":
                return true;
            case "PLASMA":
    ...
    getTechUpgradeCost(upgradeType) {
        switch (upgradeType) {
            case "SHIELDS":
                return 80;
            case "HULL":
    ...
    getShipTechUpgradeLevel(base, upgradeType) {
        switch (upgradeType) {
            case "SHIELDS":
    ...
    incrementShipTechUpgradeLevel(base, upgradeType) {
        switch (upgradeType) {
            case "SHIELDS":
                base.shieldUpgradeLevel = Math.min(SHIP_TECH_UPGRADE_MAX_LEVEL, base.shieldUpgradeLevel + 1);
                break;
            case "HULL":
                base.hullUpgradeLevel = Math.min(SHIP_TECH_UPGRADE_MAX_LEVEL, base.hullUpgradeLevel + 1);
                break;
            case "SPEED":
                base.speedUpgradeLevel = Math.min(SHIP_TECH_UPGRADE_MAX_LEVEL, base.speedUpgradeLevel + 1);
                break;
            case "RADAR":
                base.radarUpgradeLevel = Math.min(SHIP_TECH_UPGRADE_MAX_LEVEL, base.radarUpgradeLevel + 1);
                break;
            case "WEAPON":
                base.weaponUpgradeLevel = Math.min(SHIP_TECH_UPGRADE_MAX_LEVEL, base.weaponUpgradeLevel + 1);
                break;
            default:
                break;
        }
    ...
    getModuleSpawnPosition(base, moduleType) {
        if (moduleType === "WEAPON_TURRET") {
            ...
            for (const module of this.state.modules.values()) {
                if (module.baseId === base.id && module.moduleType === "WEAPON_TURRET") {
                    ...
                    const slot = Math.floor((normalizedAngle + slotSize * 0.5) / slotSize) %
